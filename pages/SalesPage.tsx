...
const SalesPage: React.FC = () => {
  ...
  return (
    <div className="bg-[#FAF9F6] text-gray-900 min-h-screen font-sans pb-32 overflow-x-hidden selection:bg-[#FE2C55]/20">
      ...
      {/* Seção Recomendação de Telas Detalhada */}
        ...
        <div className="bg-white border-2 border-[#FE2C55]/10 rounded-[3.5rem] p-8 sm:p-16 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex flex-col items-center text-center mb-12">
              <div className="bg-[#FE2C55]/10 p-5 rounded-3xl text-[#FE2C55] mb-6">
                <AlertCircle size={48} />
              </div>
              ...
              <p className="text-gray-600 text-lg leading-relaxed max-w-[800px]">
                Ela orienta que o uso seja moderado e adequado à idade da criança. Para ajudar você a entender melhor, veja abaixo os tempos de tela recomendados conforme a idade:
              </p>
            </div>
            ...
            <div className="overflow-hidden rounded-[2rem] border border-gray-100 shadow-sm bg-gray-50/50">
              <div className="bg-[#0F172A] text-white p-6 sm:p-8 flex items-center justify-between">
                <div>
                  <h4 className="text-xl font-bold">Tempo de tela</h4>
                  <p className="text-white/60 text-sm font-medium">Conforme informação da Sociedade Brasileira de Pediatria</p>
                </div>
              ...
              <div className="divide-y divide-gray-100">
                {screenTimeData.map((row, i) => (
                  <div key={i} className={`flex flex-col sm:flex-row items-center sm:items-start p-6 sm:px-8 hover:bg-white transition-colors gap-2 sm:gap-8 ${row.highlight ? 'bg-[#FE2C55]/5' : ''}`}>
                    <div className="w-full sm:w-1/3 text-[#FE2C55] font-black text-lg tracking-tight">
                      {row.age}
                    </div>
                    <div className="w-full sm:w-2/3 text-gray-700 font-bold">
                      {row.time}
                    </div>
                  </div>
                ))}
            ...
            <div className="mt-12 p-8 bg-[#FE2C55]/5 rounded-3xl border border-[#FE2C55]/10">
              <p className="text-gray-600 leading-relaxed text-center italic text-sm sm:text-base">
                O <strong>Filhos com Rotina</strong> não é para manter seu filho na frente do celular, mas sim uma ferramenta de <strong>apoio visual</strong> para que ele entenda o mundo real, diminua a ansiedade e coopere com as tarefas do dia a dia.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 px-6 bg-white border-y border-gray-100">
        <div className="max-w-[1000px] mx-auto flex flex-col md:flex-row items-center gap-12 sm:gap-20">
          <div className="relative w-full max-w-[400px]">
            ...
            <div className="absolute -bottom-6 -right-6 bg-[#FE2C55] text-white p-6 rounded-3xl shadow-xl z-20 hidden sm:block animate-bounce">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="space-y-6 text-gray-500 text-lg leading-relaxed">
              <p>Mãe e especialista em desenvolvimento infantil, Aline Neves dedicou anos para entender como a previsibilidade transforma o comportamento das crianças.</p>
              <p>Após ajudar centenas de famílias a recuperarem a paz em casa, ela condensou seu conhecimento prático em ferramentas visuais que falam a língua dos pequenos.</p>
              <p className="font-bold text-[#0F172A]">O Filhos com Rotina é o resultado dessa jornada: prático, eficiente e focado no que realmente importa: a harmonia familiar.</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl relative overflow-hidden animate-fade-in">
            ...
            <div className="p-8 sm:p-10">
              ...
              <form onSubmit={handleLeadSubmit} className="space-y-5">
                ...
                <div className="space-y-1.5"><label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-4">E-mail</label><div className="relative"><Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300" size={18} /><input type="email" required placeholder="seu@email.com" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-3xl py-4 pl-12 pr-6 text-gray-900 focus:ring-2 focus:ring-[#FE2C55]/20 focus:border-[#FE2C55] outline-none transition-all font-bold" /></div></div>
                <button type="submit" disabled={!isFormValid || isSubmitting} className={`w-full py-5 rounded-3xl font-black text-white shadow-xl transition-all flex items-center justify-center gap-3 ${isFormValid && !isSubmitting ? 'bg-[#FE2C55] shadow-[#FE2C55]/30 active:scale-95' : 'bg-gray-200 cursor-not-allowed text-gray-400'}`}>
                  {isSubmitting ? <Loader2 className="animate-spin" size={24} /> : <><span className="uppercase">Enviar Informações</span><ArrowRight size={20} /></>}
                </button>
              </form>
            </div>
          </div>
