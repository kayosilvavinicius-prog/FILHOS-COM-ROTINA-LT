    ...
    <div className="flex flex-col h-screen bg-[#F2F2F7] overflow-hidden max-w-[480px] mx-auto border-x border-gray-200 shadow-xl relative">
      ...
      {!hasStarted && (
        <div 
          ...
          className="absolute inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-6 cursor-pointer group"
        >
      ...
      <button 
        onClick={() => navigate('/')}
        className="fixed top-16 right-6 z-[90] bg-black/80 backdrop-blur-md p-3 rounded-full text-white shadow-xl active:scale-95 transition-all"
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-2 bg-[#E5DDD5] relative" 
        ...
        {messages.length === 0 && !isTyping && hasStarted && (
          <div className="flex justify-center mt-4">
             <span className="bg-white/50 backdrop-blur-sm px-3 py-1 rounded-lg text-[11px] text-gray-600 uppercase tracking-wider font-semibold">Hoje</span>
          </div>
        )}

        {messages.map((msg) => (
          <div key={msg.id} className="flex flex-col animate-fade-in">
            <div className={`max-w-[85%] px-3 py-2 rounded-lg relative text-[16px] shadow-sm ${
              msg.sender === 'character' ? 'bg-white self-start text-black rounded-tl-none' : 'bg-[#DCF8C6] self-end text-black rounded-tr-none'
            }`}>
              {msg.text}
      ...
      {showCTA && (
        <div className="absolute bottom-[100px] left-0 right-0 px-6 flex justify-center z-20 animate-fade-in">
          <button 
            onClick={() => navigate('/missao-3-video')} 
            className="w-full bg-[#25D366] text-white font-bold py-4 rounded-2xl shadow-[0_10px_30px_rgba(37,211,102,0.4)] flex items-center justify-center gap-3 active:scale-95 transition-all animate-pulse"
          >
      ...
      <footer className="bg-[#F6F6F6] border-t border-gray-300 p-2 pb-8 flex items-center gap-3">
        ...
        <div className="flex-1 bg-white border border-gray-300 rounded-full px-4 py-1.5 flex items-center text-gray-300 text-sm">
          Mensagem
        </div>
      </footer>
    </div>
